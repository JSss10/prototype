...
// Helper to format date for display (English only)
...
// Helper to parse opening hours from schema.org format
...
export default function LandmarkModal({ isOpen, onClose, onSuccess, landmark, categories }: LandmarkModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Parse detailed_information from newline-separated string to array
      const detailedInfoArray = formData.detailed_information
        .split('\n')
        .map(s => s.trim())
        .filter(s => s.length > 0)

      // Parse api_categories from comma-separated string to array
